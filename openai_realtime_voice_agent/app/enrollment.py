...
ENROLLMENT_SCRIPT = (
    ...
    "your next prompt. The wake chime is muted for this session. Tell them up "
    "front: if the ring flashes RED or the device stops listening at any point, "
    "it is harmless — just say 'hey leonard' (it re-wakes silently) and carry on "
    "where you left off. Protocol, one batch per turn: "
    ...
    "(2) Round two: five said QUICKLY and casually, as if walking past, then 'done'. "
    "(3) Round three: five said LAZILY, quietly, or mumbled, then 'done'. "
    "(4) Round four: ask them to stand across the room — five LOUDER ones from "
    "there, moving around if they like, then 'done'. "
    "(5) Round five: five more however they fancy — annoyed, amused, half-asleep — "
    "then 'done'. That is twenty-five repetitions; if a round got cut short, ask for "
    "that round again rather than counting it. "
    "(6) Then ask them to speak normally for about NINETY seconds — describe their "
    "day, read anything nearby, or ramble; encourage them briefly whenever they "
    "stop before time is up. "
    "(7) Then call voice_enrollment with action 'stop', and ONLY AFTER the tool "
    "confirms, thank them briefly and suggest — once, lightly — that a second "
    "session in another room sharpens things further. If the session drops, they "
    "can wake you again and say 'continue voice training' — start a fresh session; "
    "all takes count. Do not chat, do not explain the technology, do not comment "
